                        ...
                        <div class="row">
												...
												<div class="form-group">
													...
													<div class="col-md-3">
														<div class="input-group">
															...
															<input type="text" class="form-control" name="search" value="<?php echo $_GET['search']; ?>" placeholder="Search"> </div>
													</div>
													<div class="col-md-6">
							...
							<?php
								...
								if((strlen($date_from) > 0 && strlen($date_to) > 0) || strlen($search) > 0){
									...
									//$rows = sql($DBH, "SELECT * FROM tbl_login where date_time >= ? AND date_time <= ? AND (fullname like ? OR email like ? OR contact like ?)",
									//array($from_ts,$to_ts,"%$search%","%$search%","%$search%"), "rows");
									$rows = sql($DBH, "SELECT * FROM tbl_company 
                                    where date_time >= ? AND date_time <= ? AND name like ?", 
                                    array($from_ts,$to_ts,"%$search%"), "rows");
									...
									//$rows = sql($DBH, "SELECT * FROM tbl_login where access_level = ? ", array("admin"), "rows");
                                    $rows = sql($DBH, "SELECT * FROM tbl_company ", array(), "rows");
											...
											<tbody>
												<?php
													...
													foreach($rows as $row){
                                                        ...
                                                        $rows_login = sql($DBH, "SELECT * FROM tbl_login where company_id = ? order by (access_level) asc",
                                                        array($company_id), "rows");                                                        
                                                        $total_staff = count($rows_login);
                                                        
                                                        
                                                        $staff = array();
                                                        $staff_dp = array();
                                                        
                                                        foreach($rows_login as $row2){
                                                            $staff_id       = $row2['id'];
                                                            $staff_photo    = video_to_photo($row2['photo']);
                                                            
                                                            if($row2['access_level'] == "admin"){
                                                                $staff_type = " (Admin)";   
                                                            }else{
                                                                $staff_type = "";
                                                            }
                                                            $staff_name     = $row2['fullname'].$staff_type;
                                                            
    														$profile_url	= "employee-profile.php?id=$staff_id";//for admin   														
                                                            
                                                            
                                                            $staff_dp[] = "<a data-toggle='tooltip' title='$staff_name' style='text-decoration: none;' href='$profile_url'><img src='$staff_photo' alt='' class='user-dp' /> $fullname</a>";
                                                            $staff[]    = $staff_name;
                                                            
                                                            
                                                        }
                                                        
                                                        $staff_dp[] = "<a data-toggle='tooltip' title='Add New' style='text-decoration: none;' href='$add_user_url'><img src='../img/add_user.png' alt='' class='add-user-dp' /> $fullname</a>";
                                                            
                                                            
                                                        $staff_string = implode(", ",$staff);
                                                        $staff_dp_string = implode(" ",$staff_dp);
                                                        ...
                                                        //urls
														$profile_url		= "employee-profile.php.php?id=$id";//for admin
    													...
    													echo "<tr>
															...
															<td><a href='company.php?pid=$company_id'>$count_users_in $users_xml</a></td>
                                                            <td>$cloud_storage</td>
                                                            <td>$status_badge</td>
															<td>$permissions</td>
															<td>
                                                        ...
                                                        */
                                                        ...
                                                        echo "<tr>
															...
															<td>$name</td>
															<td>$date_time</td>
                                                            <td>$staff_dp_string <br /> <a data-toggle='tooltip' title='$staff_string' href='list_admins_more.php?id=$company_id'>View All $total_staff</a></td>
														</tr>";
													}
